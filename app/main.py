...
def read_objects(parent: Path, sha: str) -> Tuple[str, bytes]:
    ...
    bs = p.read_bytes()
    head, content = zlib.decompress(bs).split(b"\0", masxsplit=1)
    ty, _ = head.split(b" ")
...
def main():
    match sys.argv[1:]:
        ...
        case ["ls-tree", "--name-only", tree_sha]:
            items = []
            _, contents = read_objects(Path("."), tree_sha)
            while contents:
        ...
        case ["commit-tree", tree_sha, "-p", commit_sha, "-m", message]:
            contents = b"".join(
                    [
                        b"tree %b\n" % tree_sha.encode(),
                        b"parent %b\n" % commit_sha.encode(),
                        b"author singhxabhijeet <30713864+abhiiamsingh007@gmail.com> 1714599041 -0600\n",
                        b"commiter singhxabhijeet <30713864+abhiiamsingh007@gmail.com> 1714599041 -0600\n\n",
                        message.encode(),
                        b"\n",
                    ]
                )
        ...
        case ["clone", url, dir]:
            ...
            with urllib.request.urlopen(req) as f:
                ...
                body = (
                        b"011command=fetch0001000fno-progress"
                ...
                def render_tree(parent: Path, dir: Path, sha: str):
                    ...
                    _, tree = read_objects(parent, sha)
                    while tree:
